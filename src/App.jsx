...
import './index.css'
import { skills, projects, experience, hobbies, socialLinks, aboutData } from './constants/data.js'
...
function App() {
  ...
  const SkillIcon = ({ skill }) => {
    const IconComponent = {
      BarChart3, Database, Brain, Zap, Users, TrendingUp
    }[skill.icon] || BarChart3
    return <IconComponent className="w-5 h-5" />
  }

  const SocialIcon = ({ social }) => {
    const IconComponent = {
      Github, Linkedin, BookOpen, Brain, Trophy, BarChart3
    }[social.icon] || Github
    return <IconComponent className="w-5 h-5" />
  }
  ...
  const HobbyIcon = ({ hobby }) => {
    const IconComponent = {
      Camera, Music, Gamepad2, Plane, Coffee, Code
    }[hobby.icon] || Camera
